 ...
 * d'erreur HTTP cohérentes sur l'ensemble des endpoints du rapport-service.
 */
@RestControllerAdvice
public class GlobalExceptionHandler {

    @ExceptionHandler(RapportNotFoundException.class)
    public ResponseEntity<ApiErrorResponse> gererRapportNotFound(
            RapportNotFoundException ex, HttpServletRequest request) {
        return construireReponse(HttpStatus.NOT_FOUND, ex.getMessage(), request, null);
    }

    @ExceptionHandler(GenerationRapportException.class)
    public ResponseEntity<ApiErrorResponse> gererGenerationRapport(
            GenerationRapportException ex, HttpServletRequest request) {
        return construireReponse(HttpStatus.BAD_REQUEST, ex.getMessage(), request, null);
    }

    @ExceptionHandler(ServiceDistantIndisponibleException.class)
    public ResponseEntity<ApiErrorResponse> gererServiceDistantIndisponible(
            ServiceDistantIndisponibleException ex, HttpServletRequest request) {
        return construireReponse(HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage(), request, null);
    }
